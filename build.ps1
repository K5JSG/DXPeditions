...
$root = $PSScriptRoot
$project = Join-Path $root "src\DXPeditions.App\DXPeditions.App.csproj"
$publishDir = Join-Path $root "publish"
$distDir = Join-Path $root "dist"

Write-Host ""
Write-Host "DXPeditions Tracker - build v$Version" -ForegroundColor Cyan
Write-Host ("=" * 55)
...
$exe = Join-Path $publishDir "DXPeditions.App.exe"
...
if (-not $iscc) {
    ...
    Write-Host "or just distribute publish\DXPeditions.App.exe on its own." -ForegroundColor Yellow
    exit 0
}
...
& $iscc "/DMyAppVersion=$Version" (Join-Path $root "Installer\InnoSetup\DXPeditions Tracker.iss")
if ($LASTEXITCODE -ne 0) { throw "Inno Setup compilation failed." }

$setup = Join-Path $distDir "DXPeditions Tracker Setup $Version.exe"
if (Test-Path $setup) {
